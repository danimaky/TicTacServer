...
switch(msgId){
	...
	case 5:
		...
		if(salapartida.cajas[caja]="O"){
			//scket1 jugó
			if(resultado=1){
				http_get("paginaweb"+ds_map_find_value(global.usuarios,salapartida.scket1));
			}
			buffer_seek(global.buffer,buffer_seek_start,0);
			buffer_write(global.buffer,buffer_u8,msgId);
			...
			//scket2 jugó
			if(resultado=1){
				http_get("paginaweb"+ds_map_find_value(global.usuarios,salapartida.scket2));
			}
			buffer_seek(global.buffer,buffer_seek_start,0);
			buffer_write(global.buffer,buffer_u8,msgId);
			if(resultado==0||resultado==3){
