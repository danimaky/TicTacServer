...
switch (tipo_evento){
	case network_type_connect:
		...
		show_message_async(string(socket))
	...
	case network_type_disconnect:
		...
		//desconectar
		http_get("http://tictac.magtdata.com/estado.php?username="+ds_map_find_value(global.usuarios,socket)+"&estado=false");
		ds_map_delete(global.usuarios,socket);
		with(obj_message){
			instance_destroy();
		}
		var notification = instance_create_depth(0,0,1,obj_message);
		...
		if(ds_map_size(global.salas)>0){
			while(i<ds_map_size(global.salas)){
				...
				if(test.scket1==socket){
					...
					show_debug_message("La partida esta "+string(test.partida));
					if(test.partida){
						test.partida=false;
